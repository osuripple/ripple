<?php
	echo("Checking Fokabot...\n");
	$exists = $GLOBALS["db"]->fetch("SELECT * FROM users WHERE username = 'FokaBot' AND id = '999'");
	if ($exists)
		echo("Fokabot already exists, no action needed.");
	else
	{
		...
		$GLOBALS["db"]->execute("INSERT INTO `users` (`id`, `osu_id`, `username`, `password_md5`, `salt`, `email`, `register_datetime`, `rank`, `allowed`, `latest_activity`, `silence_end`, `silence_reason`, `friends`) VALUES ('999', '999', 'FokaBot', ?, ?, 'fo@kab.ot', '1452544880', '4', '1', '0', '0', '', '');", array($md5Password, base64_encode($options["salt"])));
