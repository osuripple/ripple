<?php
	...
	foreach ($scores as $score)
	{
		...
		// Find bugged scores
		$buggedScores = $GLOBALS["db"]->fetchAll("SELECT * FROM scores WHERE beatmap_md5 = ? AND username = ? AND play_mode = ? AND score < ? AND completed = 3", array($score["beatmap_md5"], $score["username"], $score["play_mode"], $score["score"]));
		if ($buggedScores)
		{
			...
			foreach ($buggedScores as $buggedScore)
			{
				...
				$GLOBALS["db"]->execute("UPDATE scores SET completed = 2 WHERE id = ?", array($buggedScore["id"]));
				$fixed++;
