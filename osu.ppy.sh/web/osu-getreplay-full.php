<?php
	...
	try
	{
		...
		// Get replay data
		$replayData = $GLOBALS["db"]->fetch("SELECT * FROM scores WHERE id = ?", array($_GET["c"]));

		// Get file name
		$beatmapName = $GLOBALS["db"]->fetch("SELECT beatmap_name FROM beatmaps_names WHERE beatmap_md5 = ?", array($replayData["beatmap_md5"]));
		if ($beatmapName)
		...
		else if ($perc300 > 0.9 && $perc40 <= 0.01 && $replayData["misses_count"] == 0)
		{
			if ($hidden)
				$rank = "SH";
			else
		...
		$magicString = md5(sprintf("%dp%do%do%dt%da%dr%de%sy%do%du%s%d%s",
			$replayData["100_count"]+$replayData["300_count"],
			$replayData["50_count"],
			$replayData["gekis_count"],
			$replayData["katus_count"],
			$replayData["misses_count"],
			$replayData["beatmap_md5"],
			$replayData["max_combo"],
			$fullCombo,
			$replayData["username"],
			$replayData["score"],
			$rank,
