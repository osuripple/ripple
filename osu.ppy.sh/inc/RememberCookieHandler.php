...
class RememberCookieHandler {
	...
	public function Check() {
		if (!empty($_COOKIE["s"]) && !empty($_COOKIE["t"])
			&& is_numeric($_COOKIE["s"]) && is_numeric($_COOKIE["t"]))
			return true;
		else
			return false;
	}
	...
	private function UpdateExisting($sid) {
		...
		$GLOBALS["db"]->execute("UPDATE remember SET token_sha = ? WHERE series_identifier = ?;", array(hash("sha256", $t), $sid));
	}
}
