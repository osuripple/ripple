...
class PasswordFinishRecovery {
	...
	public $error_messages = array("Nice troll.", "Please get your shit together and make a better password.", "barney is a dinosaur your password doesn't maaatch!", "D'ya know? your password is dumb. it's also one of the most used around the entire internet. yup.", "Don't even try.");
	public $mh_GET = array("k", "user");
	public $mh_POST = array("k", "user", "p1", "p2");
	...
	public function PrintGetData() {
		return $GLOBALS["db"]->fetch("SELECT id FROM password_recovery WHERE k = ? AND u = ?;", array($_GET["k"], $_GET["user"])) !== false;
	}
